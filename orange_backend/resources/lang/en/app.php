<?php

return [
     'App_name' => "App name",
     'Hello_admin' => "Hello admin",
     'Logout' => "Logout",
     'Main' => "Main",
     'Add_Image' => "Add Image",
     'Dashboard' => "Dashboard",
     'Users' => "Users",
     'Wallet' => "Wallet",
     'Orange' =>  "Orange",
     'Message_price' =>  "Message Price (Coins Per Message)",
     'Reverse_swipe_price' =>  "Reverse Swipe Price (Coins Per Swipe)",
     'Live_watching_price' =>  "Live Watching Price (Coins Per Minute)",
     'Login' =>  "Login",
     'Username' =>  "Username",
     'Password' =>  "Password",
     'Images' =>  "Images",
     'Add Image' =>  "Add Image",
     'Setting' =>  "Setting",
     'Full_Name' =>  "Full Name",
     'Identity' =>  "Identity",
     'Dating_or_livestream' =>  "Dating/Livestream (Keep On for Dating App)",
     'Age' =>  "Age",
     'Gender' =>  "Gender",
     'Male' =>  "Male",
     'Female' =>  "Female",
     'UserAddSuccessful' =>  "User Add Successful",
     'UserAllReadyExists' =>  "User All Ready Exists",
     'UserNotFound' =>  "User Not Found",
     'Updatesuccessful' =>  "Update successful",
     'fetchSuccessful' =>  "all data fetch successful",
     'AddPackage' =>  "Add Package",
     'Title' =>  "Title",
     'Description' =>  "Description",
     'Price' =>  "Price",
     'Months' =>  "Months",
     'Playstoreid' =>  "Android Product id",
     'Appstoreid' =>  "iOS Product id",
     'Packages' =>  "Packages",
     'Action' =>  "Action",
     'EditPackage' =>  "Edit Package",
     'AddSuccessful' =>  "Add Successful",
     'minimumPackage' =>  "minimum Package",
     'AddInterest' =>  "Add Interest",
     'Save' =>  "Save",
     'Close' =>  "Close",
     'Interests' =>  "Interests",
     'Notifications' =>  "Notifications",
     'Image' =>  "Image",
     'Edit' =>  "Edit",
     'Delete' =>  "Delete",
     'EditInterest' =>  "Edit Interest",
     'ViewDetails' =>  "View Details",
     'Report' =>  "Report",
     'Reports' =>  "Reports",
     'Reason' =>  "Reason",
     'Contact' =>  "Contact",
     'BlockUser' =>  "BlockUser",
     'Unblock' =>  "Unblock",
     'Block' =>  "Block",
     'Youtube' =>  "Youtube",
     'Instagram' =>  "Instagram",
     'Facebook' =>  "Facebook",
     'Live' =>  "Live",
     'Bio' =>  "Bio",
     'Android' =>  "Android",
     'iOS' =>  "iOS",
     'Admob' =>  "Admob",
     'InterstitialId' =>  "Interstitial Id",
     'NativeId' =>  "Native Id",
     'BannerId' =>  "Banner Id",
     'OtherSettingData' =>  "Other Setting Data",
     'Contact' =>  "Contact",
     'PrivacyPolicy' =>  "Privacy Policy",
     'TermsOfUse' =>  "Terms Of Use",
     'Appdata' =>  "App Data",
     'Currency' =>  "Currency",
     'test' =>  "test",
     'Help_support' => "Help & Support",
     'Message' => "Message",
     'Subscriptions' => "Subscriptions",
     'Add_Subscriptions' => "Add Subscriptions",
     'Lives At' => "Lives At",
     'Bio' => "Bio",
     'Edit_Subscriptions' => "Edit Subscriptions",
     'Send_notification' => "Send Notification",
     'Add_Pack' => "Add Pack",
     'Diamond_Amount' => "Diamond Amount",
     'Live_applications' => "Live Applications",
     'User' => "User",
     'Languages' => "Languages",
     'Submit' => "Submit",
     'Intro_video' => "Intro Video",
     'About' => "About",
     'Social_links' => "Social Links",
     'User_Image' => "User Image",
     'Live_eligible' => "Live Stream Eligible",
     'Live_History' => "Live History",
     'Started_At' => "Started At",
     'Streamed_For' => "Streamed For",
     'Coins_Collected' => "Coins Collected",
     'Date' => "Date",
     'Request_ID' => "Request ID",
     'Request_ID' => "Request ID",
     'Coin_Amount' => "Coin Amount",
     'Payment_Gateway' => "Payment Gateway",
     'Status' => "Status",
     'Payable_Amount' => "Payable Amount",
     'Amount_Paid' => "Amount Paid",
     'Account_details' => "Account Details",
     'Complete' => "Complete",
     'Coin_Rate' => "Coin Rate ( $ / Coin)",
     'Redeem_Requests' => "Redeem Requests",
     'View_Redeem_Requests' => "View Redeem Request",
     'All_Users' => "All Users",
     'Streamers' => "Live Streamers",
     'Pending_Requests' => "Pending Redeems",
     'Completed_Requests' => "Completed Redeems",
     'Restrict_live' => "Restrict Live-Stream",
     'Allow_live' => "Allow Live-Stream",
     'Minimum_Threshold' => "Minimum Redeem Threshold",
     'Minimum_users_needed' => "Minimum Users Needed To Keep Live Continue",
     'Maximum_minutes_for_live' => "Maximum Minutes Users Can Live Stream With < Minimum Users",
     'Diamond_packs' => "Diamond Packs",
     'Gifts' => "Gifts",
     'Add_Gift' => "Add Gift",
     'Fake_Users' => "Fake Users",
     'Add_fake_user' => "Add Fake User",
     'Blocked Users' => "Blocked Users",
     'Edit_Gift' => "Edit Gift",
     'Price_coins' => "Price (Coins)",
     'Verification_requests' => "Verification Requests",
     'Posts' => "Posts",
     'Following' => "Following",
     'Followers' => "Followers",
     'content' => "Content",
     'fullname' => "Fullname",
     'time' => "Time",
     'action' => "Action",
     'story' => "Story",
     'socialMedia' => "Social Media (Keep On for Social media)",
     'viewStoryVideo' => "View Story Video",
     'viewStory' => "View Story",
     'close' => "Close",
     'userStories' => "User Stories",
     'stories' => "Stories",
     'userPosts' => "User Posts",
     'stories' => "Stories",
     'posts' => "Posts",
     'deleteUser' => "Delete User",
     'postDescriptionLimit' => "Post Description Limit",
     'postUploadImageLimit' => "Post Upload Image Limit",
     'new_user_free_coins' => "New User Free Coins",
     'Onboarding' => "Onboarding",
     'AddOnboarding' => "Add Onboarding",
     'AddOnboardingScreen' => "Add Onboarding Screen",
     'Sortable' => "Sortable",
     'Position' => "Position",
     'EditOnboardingScreen' => "Edit Onboarding Screen",
     'Verification' => "Verification",
     'RelationshipGoals' => "Relationship Goals",
     'AddRelationshipGoal' => "Add Relationship Goal",
     'Info' => "Info",
     'EditRelationshipGoal' => "Edit Relationship Goal",
     'Religions' => "Religions",
     'Religion' => "Religion",
     'AddReligion' => "Add Religion",
     'EditReligion' => "Edit Religion",
     'Languages' => "Languages",
     'AddLanguage' => "Add Language",
     'EditLanguage' => "Edit Language",
     'allStories' => "All Stories",
     'GenderPreferred' => "Gender Preferred",
     'RelationshipGoal' => "Relationship Goal",
     'ReligionKey' => "Religion Key",
     'LanguageKeys' => "Language Keys",
     'includeFakeUserInMatching' => "Include Fake User In Matching",
     'streamAndGiftCommission' => "Stream And Gift Commission",
     'Dob' => "Date of birth"
];